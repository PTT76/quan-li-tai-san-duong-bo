...
> **Tài liệu dùng để duyệt thiết kế, chưa phải giao diện sản xuất.** Toàn bộ tên tuyến, mã hồ sơ, người dùng, tọa độ, số tiền, tiến độ và KPI xuất hiện trong prototype đều là dữ liệu minh họa. Không sử dụng tài liệu này làm căn cứ nghiệp vụ, pháp lý hoặc tài chính.

## 1. Mục tiêu vòng duyệt

Vòng draft này kiểm tra kiến trúc thông tin, mức độ dễ hiểu của luồng công việc và ngôn ngữ giao diện trước khi lập kế hoạch triển khai Next.js. Prototype tĩnh được đặt trong `ui-ux-draft`; không có backend, không gọi API bản đồ và không ghi dữ liệu thật. Mỗi ảnh có thể tái tạo bằng URL `ui-ux-draft/index.html?screen=<MÃ_FRAME>&state=default`. Điều hướng giữa frame, tab, toast và một số thao tác cục bộ như retry được mô phỏng để hỗ trợ review; bảy kịch bản được duyệt bằng chuỗi URL ổn định, không phải một state machine nghiệp vụ hoàn chỉnh.
...
| GIS | Bản đồ vector mô phỏng; có legend, layer preset, bộ lọc thời gian, độ tin cậy vị trí và đồng bộ lựa chọn giữa các view |
...
- **Dữ liệu minh họa:** KPI sự cố mở, tỷ lệ phủ tuần, giải ngân; bản đồ điểm nóng; danh sách “Cần quyết định hôm nay”; sai lệch kế hoạch–thực tế.
...
- **Dữ liệu minh họa:** vệt ca tuần, sự cố, tài sản, dự án và quy hoạch trên bản đồ; queue `Khẩn`, `Chờ xác minh`, `Ca chưa bắt đầu`, `Công việc chậm`.
...
- **Trạng thái minh họa:** cùng một tài sản được chọn đồng thời ở `Bản đồ | Bình đồ | Bảng`; bộ lọc tuyến được giữ nguyên khi đổi view.
- **Requirement ID:** `D01`, `D03`, `C03`, `G3.02`.
- **Dữ liệu minh họa:** mặt đường, cầu/cống, biển báo, tình trạng, lớp quy hoạch, lý trình, lịch sử bảo trì và độ tin cậy vị trí.
...
- **Dữ liệu minh họa:** hình học trước/sau, hệ tọa độ, nguồn đo, sai số, điểm snap, trùng tiềm năng, lịch sử phiên bản và phạm vi đợt khảo sát.
...
- **Trạng thái minh họa:** dataset công khai đã duyệt, có thời điểm cập nhật; chi tiết chỉ chứa trường được phép công bố.
- **Requirement ID:** `B02`, `G6.02`.
- **Dữ liệu minh họa:** tuyến, phạm vi ảnh hưởng, loại sự cố công khai, trạng thái xử lý đã làm mờ thông tin nội bộ và nguồn cập nhật.
...
- **Giả định cần xác nhận:** bộ trường công khai, độ trễ xuất bản, quy trình gỡ/sửa, tuyên bố miễn trừ và việc người dân chỉ xem hay được gửi phản ánh.
...
- **Dữ liệu minh họa:** thời lượng/đoạn đã đi, vệt GPS, Km+m dự kiến, độ chính xác, ảnh/video, thời gian thiết bị và trạng thái upload.
...
- **Giả định cần xác nhận:** tần suất tracking, retention, điều kiện hoàn thành ca, số media/tệp, số điện thoại/kênh dự phòng và quy tắc cảnh báo khẩn.
...
- **Dữ liệu minh họa:** tuyến/Km+m tự tính, tọa độ, sai số, nguồn đo, ảnh, thuộc tính tài sản, ứng viên trùng và lý do hiệu chỉnh.
- **Hành động chính:** đo lại; chọn tài sản hiện hữu hoặc tạo ứng viên mới; sửa snap kèm lý do; lưu offline; gửi sang hàng đợi QA.
- **Giả định cần xác nhận:** ngưỡng sai số/chặn, bán kính phát hiện trùng, thuộc tính bắt buộc, thiết bị độ chính xác cao và quyền hợp nhất.
...
- Chỉ dùng fixture dữ liệu minh họa tại local; không database, không auth thật, không API key, không gọi bản đồ bên ngoài.
- Tạo map/straight-line diagram bằng SVG/CSS/vector giả lập.
- Có thể chạy bằng các script chuẩn dev/build/lint; giao diện không phụ thuộc dịch vụ trả phí.

NGÔN NGỮ VÀ PHONG CÁCH
- Toàn bộ UI là tiếng Việt tự nhiên, UTF-8, dấu đúng; tên/mã/tuyến/số tiền chỉ là dữ liệu demo.
...
- Không triển khai backend, auth production, tích hợp thật, gửi notification thật hoặc logic pháp lý thật.
```
...
- Chỉ dùng mock/fixture local; không Supabase/database/auth thật, không API key, không map service bên ngoài, không secret.
- Bản đồ và bình đồ là SVG/CSS/vector mô phỏng.
...
- UI 100% tiếng Việt tự nhiên, UTF-8; mọi tên, tọa độ, số tiền, KPI là dữ liệu minh họa và phải có nhãn nhận biết.
...
- Avoid real people, exact production coordinates, credentials or unapproved financial values.
...
- No production backend, real auth, real notification, real connector, production backup, regulatory logic or deployment architecture claim.
```
